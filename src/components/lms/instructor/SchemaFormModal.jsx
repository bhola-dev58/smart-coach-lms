            ...
            {config.fields.map(field => (
              <div key={field.key} style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                ...
                {field.type === 'textarea' ? (
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '6px', border: '1px solid var(--dash-border)', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    {(formData[field.key] || []).map((faq, idx) => (
                       <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem', paddingBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                         <input 
                           ...
                           style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid var(--dash-border)', borderRadius: '4px', padding: '0.5rem', color: 'white' }}
                         />
                         <textarea 
                           placeholder="Answer" value={faq.answer} rows={2}
                           onChange={e => {
                             const newFaqs = [...(formData[field.key] || [])];
                         ...
                         <button type="button" onClick={() => {
                             ...
                             handleChange(field.key, newFaqs);
                         }} style={{ alignSelf: 'flex-end', background: 'transparent', color: '#ef4444', border: 'none', cursor: 'pointer', fontSize: '0.8rem' }}>Remove FAQ</button>
                       </div>
                    ))}
                    <button type="button" onClick={() => {
                       const newFaqs = [...(formData[field.key] || [])];
                       ...
                       handleChange(field.key, newFaqs);
                    }} style={{ width: 'fit-content', padding: '0.4rem 0.8rem', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.85rem' }}>+ Add FAQ</button>
                  </div>
                ) : (
                  <input 

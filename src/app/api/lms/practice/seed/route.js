...
const STATIC_QUESTION_BANK = {
  ...
  SCIENCE: [
    ...
    { q: 'The speed of light is approximately:', options: ['3 × 10⁶ m/s', '3 × 10⁸ m/s', '3 × 10¹0 m/s', '3 × 10⁴ m/s'], ans: 1 },
...
export async function POST() {
  try {
    ...
    // Find the demo instructor user
    let instructor = await User.findOne({ email: 'instructor@gradify.academy' });
    if (!instructor) {
    ...
    if (!instructor) {
      return NextResponse.json({ success: false, message: 'No instructor user found to assign questions' }, { status: 404 });
    }

    // Clear existing practice questions to prevent duplicates
    await PracticeQuestion.deleteMany({ createdBy: instructor._id });

    // Seed questions
    const insertData = [];
    for (const [subjectKey, questionList] of Object.entries(STATIC_QUESTION_BANK)) {
    ...
    const result = await PracticeQuestion.insertMany(insertData);

    return NextResponse.json({
      success: true,
      message: `Seeded ${result.length} practice questions successfully!`,
      instructorEmail: instructor.email,
    });
  } catch (error) {
    console.error('[POST /api/lms/practice/seed]', error);
